...
class CI_Image_lib {
	...
	function initialize($props = array())
	{
		...
		/*
		 * Is get<?php echo base_url();?>ize() Available?
		 *
		 ...
		 *
		 */
		if ( ! function_exists('get<?php echo base_url();?>ize'))
		{
	...
	function image_process_gd($action = 'resize')
	{
		...
		if ($this->image_type == 3) // png we can actually preserve transparency
		{
			...
			<?php echo base_url();?>avealpha($dst_img, TRUE);
		}
		...
		if ($this->rotation_angle == 'hor')
		{
			for ($i = 0; $i < $height; $i++)
			{
				...
				while ($left < $right)
				{
					...
					<?php echo base_url();?>etpixel($src_img, $left, $i, $cr);
					<?php echo base_url();?>etpixel($src_img, $right, $i, $cl);
		...
		else
		{
			for ($i = 0; $i < $width; $i++)
			{
				...
				while ($top < $bot)
				{
					...
					<?php echo base_url();?>etpixel($src_img, $i, $top, $cb);
					<?php echo base_url();?>etpixel($src_img, $i, $bot, $ct);
	...
	function overlay_watermark()
	{
		...
		{
			// set our RGB value from above to be transparent and merge the <?php echo base_url();?> with the specified opacity
	...
	function text_watermark()
	{
		...
		{
			if ($this->wm_use_drop_shadow)
				<?php echo base_url();?>tring($src_img, $this->wm_font_size, $x_shad, $y_shad, $this->wm_text, $drp_color);
				<?php echo base_url();?>tring($src_img, $this->wm_font_size, $x_axis, $y_axis, $this->wm_text, $txt_color);
		}
	...
	function get_image_properties($path = '', $return = FALSE)
	{
		...
		$vals = @get<?php echo base_url();?>ize($path);
	 ...
	 * source_<?php echo base_url();?> with multiple periods, like:  my.cool.jpg
