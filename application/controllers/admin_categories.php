	...
	public function index()
	{
		...
		if($search_string !== false && $order !== false || $this->uri->segment(3) == true){
            ...
            */
			...
			$data['count_products']= $this->categories_model->count_categories($search_string, $order);
			...
			if($search_string){
				...
				if($order){
					$data['categoriess'] = $this->categories_model->get_categories('', $order, $order_type, $config['per_page'],$limit_end);
				}else{
			...
			$filter_session_data['categories_selected'] = null;
			...
			//fetch sql data into arrays
			$data['count_products']= $this->categories_model->count_categories();
			$data['categories'] = $this->categories_model->get_categories('', '', $order_type, $config['per_page'],$limit_end);
			$config['total_rows'] = $data['count_products'];
	...
	public function add()
	{
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST')
		{
			...
			if ($this->form_validation->run())
			{
				...
				if($this->categories_model->store_categories($data_to_store)){
	...
	public function update()
	{
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST')
		{
			...
			if ($this->form_validation->run())
			{
				...
				//if the insert has returned true then we show the flash message
				if($this->categories_model->update_categories($id, $data_to_store) == TRUE){
